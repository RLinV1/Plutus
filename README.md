# Stock Research Assistant — AI-native, beginner-friendly
...
2. **Autonomous agent** — a Claude agent that calls the MCP server's tools to
   answer natural-language questions and cite sources.
3. **Eval harness** — a test-and-self-correct loop that scores the agent's tool
...
> **Educational only — not investment advice.** It cannot predict prices.
...
plain language — translating jargon (market cap, P/E, the 200-day moving average,
...
|---|---|
| `get_stock_snapshot` | "Tell me about Apple" — name, sector, price, daily move, market cap, P/E |
| `get_price_performance` | "How has Nvidia done this year?" — % return + "calm/average/bumpy" |
| `compare_stocks` | "Compare Apple and Microsoft" — side-by-side basics |
| `explain_stock_risk` | "Is Tesla risky?" — beta + volatility in plain words |
| `get_technical_indicators` | "Is Apple above its 200-day average?" — SMAs, RSI, 52-week range |
| `get_ticker_news` | "What's the latest on Apple?" — recent headlines (free, keyless) |
| `explain_price_move` | "Why did Apple drop?" — the move + same-window news that may explain it |
| `get_filing_risks` | "What are the risks in Apple's 10-K?" — SEC risk factors in plain English |
| `get_watchlist_digest` | "What moved on my watchlist?" — per-ticker move + headlines (daily digest) |
| `search_knowledge` | "What's a P/E ratio?" / "Why invest in Nvidia?" — cited RAG lookups |
...
```
 ...
 ───────────────────            ──────────────────────────        ──────
 agent/client.py        ──►     server/mcp_server.py        ──►   tools.py
   • real: tool_runner            @mcp.tool wrappers                ├─ risk/metrics.py
   • mock: mock_agent.py          (thin; delegate to tools.py)      ├─ risk/indicators.py
                                                                    ├─ data/market_data.py (live yfinance | mock)
                                                                    │   ├─ get_prices()  + parquet cache
                                                                    │   └─ get_company_info() + JSON cache
                                                                    └─ rag/ (Chroma + MiniLM)
 evals/run_evals.py  ── scores agent vs. ground truth from the same engine (mock data)
```

**Key design choice:** tool logic lives once in `src/portfolio_risk/tools.py`.
The MCP server, the mock agent, and the tests all call it — so the eval's
...
average?" needs a live number → the agent calls the `get_technical_indicators`
**MCP tool**. "What *is* a 200-day average?" needs a stable explanation → the
...
```powershell
# Ask the assistant from the CLI (mock agent — no API key needed)
...
| `EDGAR_USER_AGENT` | — | Only for `rag.ingest --edgar` (SEC requires a descriptive UA). |
...
> **Can I run Claude locally / via Ollama?** No — Claude is a closed-weight model,
> available only through the Anthropic API (or Bedrock/Vertex). Ollama runs
> open-weight models (Llama, Mistral, Qwen, …). The mock agent is the offline
...
- **stdio servers must never print to stdout** — it corrupts JSON-RPC. All logs
...
- **yfinance is flaky** — live prices/info are cached; tests and evals set
...
- Mock prices and company info are **pure-from-hash** so they're identical every
  run — that's what keeps the evals reproducible.
